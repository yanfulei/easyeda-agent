...
func AllActions() []ActionSpec {
	actions := []ActionSpec{
		...
		},
		{
			...
			ChangesContext: true,
			NeedsWindow: true,
			Description: "Select schematic primitives by id and return the active selection. Selection is editor context state, so this action is serialized with writes and quarantined after an abandoned mutation.",
			Inputs:      []string{"primitiveIds"},
