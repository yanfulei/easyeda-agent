...
**影响**：它被 `zone-arrange` 当成 J2 的一支 GND(left) 旗，直接触发上面 #6 的 R5 拒绝。
我删掉它之前，怎么调 J2 的朝向都没用——**排查成本远高于 bug 本身**。

**修法建议**：探测后立刻删；退一步，至少让所有几何判据把 `__ROTPROBE__` 排除在外。
